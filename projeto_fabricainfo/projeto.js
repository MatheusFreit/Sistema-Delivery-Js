...
let opcoes = [
  ...
  "Cliente com mais pedidos no mês",//feito
  "Clientes com mais pedidos no mês", //feito
  "Cliente com mais pedidos em um mês específico", //feito
  "Calcular bônus cliente", //feito
  "Funcionário com maior número de vendas no mês", //feito
  "Entregador com maior número de entregas no mês" //feito
  ];
  ...
  do {
    ...
    switch (op) {
      ...
      case 14:
        ...
        break;
      case 15:
        ConsultarClienteComMaisPedidos();
        break;
      case 16:
        ConsultarClientesComMaisPedidos();
        break;
      case 17:
        ClienteComMaisPedidosNoMes();
        break;      
      case 18:
...
function ConsultarClienteComMaisPedidos() {
  const mesAtual = new Date().getMonth() + 1; // Obtém o número do mês atual (adiciona 1, pois os meses são indexados a partir de zero)

  // Criar um objeto para armazenar o número de pedidos de cada cliente
  ...
  for (let i = 0; i < ListaPedidos.length; i++) {
    ...
    // Verificar se o pedido foi feito no mês atual
    const mesPedido = new Date(pedido.data).getMonth() + 1; // Obtém o número do mês do pedido
    if (mesPedido === mesAtual) {
      const cliente = pedido.cliente;

      // Verificar se o cliente já possui pedidos contabilizados
      if (pedidosPorCliente[cliente.cpf]) {
        // Incrementar o número de pedidos do cliente
        pedidosPorCliente[cliente.cpf]++;
      } else {
        // Inicializar o número de pedidos do cliente como 1
        pedidosPorCliente[cliente.cpf] = 1;
      }
    }
  }

  // Verificar qual cliente possui o maior número de pedidos
  let clienteComMaisPedidos = null;
  let maiorNumeroPedidos = 0;

  for (const cpfCliente in pedidosPorCliente) {
    const numeroPedidos = pedidosPorCliente[cpfCliente];

    if (numeroPedidos > maiorNumeroPedidos) {
      maiorNumeroPedidos = numeroPedidos;

      // Encontrar o cliente correspondente ao CPF
      clienteComMaisPedidos = ListaCliente.find(cliente => cliente.cpf === cpfCliente);
    }
  }

  // Verificar se algum cliente foi encontrado com pedidos no mês atual
  if (clienteComMaisPedidos) {
    console.log(`O cliente com mais pedidos no mês atual é: ${clienteComMaisPedidos.nome} (CPF: ${clienteComMaisPedidos.cpf}).`);
    console.log(`Número de pedidos: ${maiorNumeroPedidos}`);
  } else {
    console.log("Nenhum cliente fez pedidos no mês atual.");
  }
}

function ConsultarClientesComMaisPedidos() {
  const mesAtual = new Date().getMonth() + 1; // Obtém o número do mês atual (adiciona 1, pois os meses são indexados a partir de zero)
  ...
  // Criar uma lista de clientes com seus respectivos números de pedidos
  ...
  for (const cpfCliente in pedidosPorCliente) {
    ...
    // Encontrar o cliente correspondente ao CPF
    const cliente = ListaCliente.find(cliente => cliente.cpf === cpfCliente);

    clientesComPedidos.push({
      cliente: cliente,
      numeroPedidos: numeroPedidos
    });
  }

  // Ordenar a lista de clientes por número de pedidos em ordem decrescente
  clientesComPedidos.sort((a, b) => b.numeroPedidos - a.numeroPedidos);

  // Exibir os clientes com mais pedidos
  if (clientesComPedidos.length > 0) {
    console.log("Clientes com mais pedidos no mês atual:");

    for (let i = 0; i < clientesComPedidos.length; i++) {
      const cliente = clientesComPedidos[i].cliente;
      const numeroPedidos = clientesComPedidos[i].numeroPedidos;

      console.log(`${cliente.nome} (CPF: ${cliente.cpf}) - Número de pedidos: ${numeroPedidos}`);
    }
  } else {
    console.log("Nenhum cliente fez pedidos no mês atual.");
  }
...
function ClienteComMaisPedidosNoMes() {
  // Criar um objeto para armazenar o número de pedidos de cada cliente
  const pedidosPorCliente = {};
  ...
  for (let i = 0; i < ListaPedidos.length; i++) {
    ...
    // Verificar se o pedido foi feito no mês especificado
    const mesPedido = new Date(pedido.data).getMonth() + 1; // Obtém o número do mês do pedido
    if (mesPedido === mes) {
      const cliente = pedido.cliente;
  ...
  for (const cpfCliente in pedidosPorCliente) {
    ...
    // Verificar se o cliente tem mais pedidos do que o atual máximo
    if (numeroPedidos > maxPedidos) {
      maxPedidos = numeroPedidos;
  ...
  // Verificar se foi encontrado um cliente com mais pedidos
  if (clienteComMaisPedidos !== null) {
    console.log(`Cliente com mais pedidos no mês ${mes}: ${clienteComMaisPedidos.nome} (CPF: ${clienteComMaisPedidos.cpf}) - Número de pedidos: ${maxPedidos}`);
  } else {
    console.log(`Nenhum cliente fez pedidos no mês ${mes}.`);
  }

  return clienteComMaisPedidos;
}
